    ...
    updateQuantity(line, quantity, name, variantId) {
      ...
      fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
        ...
        .finally(() => {
          ...
          const totalValueElement = document.querySelector('.totals__total-value span');
          if (!totalValueElement) {
 
            return;
          }
          const totalValue = totalValueElement.textContent.trim();
          const totalValueNumber = parseFloat(totalValue.replace(/[^0-9.-]+/g, '')).toFixed(2);
          console.log('totalValueNumber', totalValueNumber);
          console.log('updateQuantity', name);
          const parser = new DOMParser();

          const items = document.querySelectorAll('.cart-item');
          const itemIds = Array.from(items).map(item => item.getAttribute('data-id'));
          const targetVariantId = cart_free_product_item;
          const targetVariantPosition = itemIds.indexOf(targetVariantId);
          ...
          function removeFromCart(line) {
            ...
            .then(cart => {
              ...
              location.reload(); // Refresh the page or update the cart UI
            })
            .catch(error => console.error('Error:', error));
          }

          console.log('cart_freegift_amount:', totalValueNumber < cart_freegift_amount ,      totalValueNumber, cart_freegift_amount ,itemIds.includes(targetVariantId));
   
          if (totalValueNumber < cart_freegift_amount && itemIds.includes(targetVariantId)) {
            console.log('cart_freegift_amount:', cart_freegift_amount);
            // Call removeFromCart and pass targetVariantPosition
            const progressBar = document.querySelector('.progress-bar');
            if (progressBar) {
            progressBar.classList.remove('progress-compleate');
            }

            console.log('Target variant position:', targetVariantPosition, name);
            if (targetVariantPosition !== -1) {
              removeFromCart(targetVariantPosition + 1 ); 
            }
          }
          ...
          if (totalValueNumber > cart_freegift_amount && !itemIds.includes(targetVariantId)) {
            ...
            const progressBar = document.querySelector('.progress-bar');
            if (progressBar) {
            progressBar.classList.add('progress-compleate');
            }
